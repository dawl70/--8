...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnAnalyze.addEventListener('click', () => {
        ...
        document.getElementById('result-title').textContent = `${displayName}님의 사주명식`;
        document.getElementById('summary-name').textContent = displayName;
        ...
        if (lunarNotice) {
            document.getElementById('result-title').textContent = `${displayName}님의 사주명식${lunarNotice}`;
        }
    ...
    function updateSajuTable(year, month, day) {
        ...
        const branches = [
            ...
            //         yc(계산): 子=壬=양,丑=己=음,寅=甲=양,卯=乙=음,辰=戊=양,巳=丙=양,午=丁=음,未=己=음,申=庚=양,酉=辛=음,戌=戊=양,亥=壬=양
            { h:'子',g:'자',e:'water',y:'yang',yc:'yang' }, { h:'丑',g:'축',e:'earth',y:'yin', yc:'yin'  },
        ...
        const hInt = parseInt(document.getElementById('hour').value) || 12;
        const mSelectInt = parseInt(document.getElementById('minute').value) || 30;
        ...
        const getInteractions = () => {
            ...
            for (let i = 0; i < bArr.length; i++) {
                for (let j = i + 1; j < bArr.length; j++) {
                    pairs.push([bArr[i], bArr[j]]);
                }
            }

            const yukHap = ['子丑', '丑子', '寅亥', '亥寅', '卯戌', '戌卯', '辰酉', '酉辰', '巳申', '申巳', '午未', '未午'];
            const samHyeong = ['寅巳', '巳申', '寅申', '丑戌', '戌未', '丑未', '子卯', '卯子'];
            const jaHyeong = ['辰辰', '午午', '酉酉', '亥亥'];
            const yukChung = ['子午', '午子', '丑未', '未丑', '寅申', '申寅', '卯酉', '酉卯', '辰戌', '戌辰', '巳亥', '亥巳'];
            const yukPa = ['子酉', '酉子', '丑辰', '辰丑', '寅亥', '亥寅', '卯午', '午卯', '巳申', '申巳', '戌未', '未戌'];
            const yukHae = ['子未', '未子', '丑午', '午丑', '寅巳', '巳寅', '卯辰', '辰卯', '申亥', '亥申', '酉戌', '戌酉'];

            pairs.forEach(([p1, p2]) => {
                const comb1 = p1.h + p2.h;
                const comb2 = p2.h + p1.h;

                if (yukHap.includes(comb1)) haps.push(`육합(${p1.h}${p2.h})`);
                if (yukChung.includes(comb1)) chungs.push(`충(${p1.h}${p2.h})`);
                if (yukPa.includes(comb1)) pas.push(`파(${p1.h}${p2.h})`);
                if (yukHae.includes(comb1)) haes.push(`해(${p1.h}${p2.h})`);
                
                if (samHyeong.includes(comb1) || samHyeong.includes(comb2)) {
                    hyeongs.push(`형(${p1.h}${p2.h})`);
                }
            });

            const count = {};
            bArr.forEach(b => count[b.h] = (count[b.h] || 0) + 1);
            jaHyeong.forEach(jh => {
                const char = jh[0];
                if (count[char] >= 2) {
                    hyeongs.push(`자형(${char}${char})`);
                }
            });

            const samHapGroup = ['亥卯未', '寅午戌', '巳酉丑', '申子辰'];
            samHapGroup.forEach(sh => {
                const matched = [...new Set(bArr.filter(b => sh.includes(b.h)).map(b => b.h))];
                if (matched.length >= 3) {
                    haps.push(`삼합(${sh})`);
                } else if (matched.length === 2) {
                    haps.push(`반합(${matched.join('')})`);
                }
            });

            const allInteractions = [...haps, ...hyeongs, ...chungs, ...pas, ...haes];
        ...
        const getStemInteractions = () => {
            ...
            for (let i = 0; i < sArr.length; i++) {
                for (let j = i + 1; j < sArr.length; j++) {
                    pairs.push([sArr[i], sArr[j]]);
                }
            }

            const cheonHap = ['甲己', '己甲', '乙庚', '庚乙', '丙辛', '辛丙', '丁壬', '壬丁', '戊癸', '癸戊'];
            const cheonChung = ['甲庚', '庚甲', '乙辛', '辛乙', '丙壬', '壬丙', '丁癸', '癸丁'];

            pairs.forEach(([p1, p2]) => {
                const comb1 = p1.h + p2.h;
                if (cheonHap.includes(comb1)) haps.push(`합(${p1.h}${p2.h})`);
                if (cheonChung.includes(comb1)) chungs.push(`충(${p1.h}${p2.h})`);
            });
        ...
        const summaryName = document.getElementById('summary-name').textContent;
        ...
        const renderLifeFortune = (dateObj) => {
            ...
            const dirMap = { 'wood': '동쪽(東)', 'fire': '남쪽(南)', 'earth': '중앙/현위치(中)', 'metal': '서쪽(西)', 'water': '북쪽(北)' };
            let yDirs = [];
            yongheeElements.split(',').forEach(part => {
                const k = part.trim()[0];
                if (k && korToEng[k]) yDirs.push(dirMap[korToEng[k]]);
            });

            const isaDesc = document.getElementById('isa-desc');
            if (isaDesc) {
                isaDesc.innerHTML = `선택하신 날(${curDate.getMonth()+1}/${curDate.getDate()})은 <strong>${curDayStem.h}${curDayBranch.h}일</strong>입니다.<br>사주에 맞는 길한 이동 방향은 <strong>${yDirs.join(', ')}</strong>입니다. 이 날 이사나 장거리 이동을 계획하신다면 이 방향을 우선 고려하세요.`;
            }

            const dirDesc = document.getElementById('direction-desc');
            if (dirDesc) {
                dirDesc.innerHTML = `일상에서 행운을 부르는 방위 역시 <strong>${yDirs.join(', ')}</strong>입니다. 선택한 날 중요한 계약이나 미팅이 있다면 해당 방향을 향해 앉는 것이 유리합니다.`;
            }
            ...
            if (dateDesc) {
                ...
                dateDesc.innerHTML = `선택하신 날짜의 택일 점수는 <strong style="color: ${color};">${evalText}</strong>입니다.<br>본인의 일간과 상생하는 기운(${yongheeElements})을 기준으로 산출된 결과입니다.`;
            }
        };
    ...
    // 9. Compatibility Analysis Logic (Integrated into Love Tab)
    const btnAnalyzeComp = document.getElementById('btn-analyze-compatibility-v2');
    const compatibilityResult = document.getElementById('compatibility-result-v2');
    ...
    if (btnAnalyzeComp) {
        btnAnalyzeComp.addEventListener('click', () => {
            ...
            // Simulate loading
            compatibilityResult.innerHTML = '<div class="loading-spinner"><i class="fa-solid fa-spinner fa-spin"></i> 궁합 분석 중...</div>';

            setTimeout(() => {
                compatibilityResult.innerHTML = `
                    <div class="analysis-section" style="width: 100%; text-align: left;">
                        <div style="text-align: center; margin-bottom: 25px;">
                            <div style="font-size: 2.8rem; font-weight: 800; color: #ff4757; text-shadow: 0 0 15px rgba(255, 71, 87, 0.3); margin-bottom: 5px;">85%</div>
                            <div style="font-size: 0.9rem; color: var(--text-muted); font-weight: 500;">천생연분의 기운</div>
                        </div>

                        <!-- 상세 조화도 지표 -->
                        <div style="margin-bottom: 25px; background: rgba(255, 255, 255, 0.03); padding: 15px; border-radius: 12px;">
                            <div style="margin-bottom: 12px;">
                                <div style="display: flex; justify-content: space-between; font-size: 0.75rem; margin-bottom: 5px; color: var(--text-muted);"><span>애정 에너지</span><span>92%</span></div>
                                <div style="height: 6px; background: rgba(255,255,255,0.1); border-radius: 3px; overflow: hidden;"><div style="width: 92%; height: 100%; background: linear-gradient(90deg, #ff4757, #ff7f50);"></div></div>
                            </div>
                            <div style="margin-bottom: 12px;">
                                <div style="display: flex; justify-content: space-between; font-size: 0.75rem; margin-bottom: 5px; color: var(--text-muted);"><span>소통 및 우정</span><span>78%</span></div>
                                <div style="height: 6px; background: rgba(255,255,255,0.1); border-radius: 3px; overflow: hidden;"><div style="width: 78%; height: 100%; background: linear-gradient(90deg, #3498db, #2ecc71);"></div></div>
                            </div>
                            <div>
                                <div style="display: flex; justify-content: space-between; font-size: 0.75rem; margin-bottom: 5px; color: var(--text-muted);"><span>신뢰 및 안정</span><span>85%</span></div>
                                <div style="height: 6px; background: rgba(255,255,255,0.1); border-radius: 3px; overflow: hidden;"><div style="width: 85%; height: 100%; background: linear-gradient(90deg, #f1c40f, #e67e22);"></div></div>
                            </div>
                        </div>

                        <div class="analysis-card" style="background: rgba(255, 71, 87, 0.1); border: 1px solid #ff4757; padding: 15px; border-radius: 10px; margin-bottom: 15px;">
                            <p style="font-weight: 700; color: #ff4757; margin-bottom: 5px;">[인연 총평]</p>
                            <p style="color: var(--text-main); font-size: 0.9rem; line-height: 1.5;">"<strong>${compName}</strong>님과는 서로의 부족한 오행을 채워주는 상보적인 관계입니다. 특히 정신적인 교감이 매우 뛰어나 함께 있을 때 큰 안정을 얻습니다."</p>
                        </div>
                        <ul style="list-style: none; padding: 0; font-size: 0.85rem; color: var(--text-muted); line-height: 1.8;">
                            <li><i class="fa-solid fa-circle-check" style="color: #ff4757; margin-right: 8px;"></i> <strong>성격:</strong> 서로의 다름을 존중하며 배울 점이 많습니다.</li>
                            <li><i class="fa-solid fa-circle-check" style="color: #ff4757; margin-right: 8px;"></i> <strong>인연:</strong> 전생부터 이어지는 깊은 인연의 끈이 보입니다.</li>
                        </ul>

                        <div style="margin-top: 15px; background: rgba(255, 71, 87, 0.05); padding: 15px; border-radius: 12px; border-left: 4px solid #ff4757;">
                            <h4 style="font-size: 0.9rem; color: var(--text-main); margin-bottom: 8px;"><i class="fa-solid fa-lightbulb" style="color: #ff4757;"></i> 두 분을 위한 관계 조언</h4>
                            <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.6;">"서로의 장점을 키워주는 '상생'의 관계입니다. 대화할 때 결론을 내기보다 상대의 감정에 먼저 공감해 보세요. 특히 기념일이나 작은 선물로 마음을 표현하면 인연의 끈이 더욱 단단해질 것입니다."</p>
                        </div>

                        <!-- 결혼시기 및 택일 분석 -->
                        <div style="margin-top: 20px; background: rgba(255, 255, 255, 0.02); padding: 15px; border-radius: 12px; border: 1px solid rgba(255, 71, 87, 0.2);">
                            <h4 style="font-size: 0.9rem; color: #ff4757; margin-bottom: 12px;"><i class="fa-solid fa-ring"></i> 결혼운 및 택일 가이드</h4>
                            <div style="display: grid; grid-template-columns: 1fr; gap: 10px;">
                                <div style="background: rgba(255, 71, 87, 0.03); padding: 10px; border-radius: 8px;">
                                    <div style="font-size: 0.8rem; color: var(--text-main); font-weight: 700; margin-bottom: 4px;">추천 결혼 시기</div>
                                    <p style="font-size: 0.75rem; color: var(--text-muted);">두 분의 합(合)이 강하게 들어오는 <strong>2027년(丁未)</strong> 혹은 <strong>2029년(己酉)</strong>이 결실을 맺기에 가장 좋습니다.</p>
                                </div>
                                <div style="background: rgba(255, 71, 87, 0.03); padding: 10px; border-radius: 8px;">
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                                        <div style="font-size: 0.8rem; color: var(--text-main); font-weight: 700;">결혼 택일 검증</div>
                                        <input type="date" id="marriage-fortune-date" class="saju-input" style="width: 140px; padding: 2px 6px; font-size: 0.75rem; height: 26px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,71,87,0.3); color: #fff;">
                                    </div>
                                    <p style="font-size: 0.75rem; color: var(--text-muted);" id="marriage-fortune-result">날짜를 선택하시면 해당 날짜의 결혼 궁합 길흉을 분석해 드립니다.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-top: 20px; display: grid; grid-template-columns: 1fr; gap: 10px;">
                            <div style="background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 10px; border: 1px solid rgba(255, 71, 87, 0.1);">
                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
                                    <i class="fa-solid fa-fire-alt" style="color: #ff4757;"></i>
                                    <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-main);">속궁합 (에너지 조화)</span>
                                </div>
                                <p style="font-size: 0.8rem; color: var(--text-muted);">신체적, 에너지적 리듬이 매우 잘 맞아 서로에게 강력한 끌림을 느끼는 최상의 조합입니다.</p>
                            </div>
                            <div style="background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 10px; border: 1px solid rgba(255, 71, 87, 0.1);">
                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
                                    <i class="fa-solid fa-ring" style="color: #ff4757;"></i>
                                    <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-main);">결혼 가능성</span>
                                    <span style="margin-left: auto; color: #ff4757; font-weight: 800;">90%</span>
                                </div>
                                <p style="font-size: 0.8rem; color: var(--text-muted);">가정관과 가치관이 일치하여 장기적인 결실을 맺기에 매우 유리한 운세를 가지고 있습니다.</p>
                            </div>
                            <div style="background: rgba(255, 255, 255, 0.03); padding: 12px; border-radius: 10px; border: 1px solid rgba(52, 152, 219, 0.2);">
                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 5px;">
                                    <i class="fa-solid fa-handshake" style="color: #3498db;"></i>
                                    <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-main);">사업 파트너 궁합</span>
                                    <span style="margin-left: auto; color: #3498db; font-weight: 800;">75%</span>
                                </div>
                                <p style="font-size: 0.8rem; color: var(--text-muted);">공적인 업무에서는 이성적인 소통이 필요하지만, 서로의 단점을 보완해 주는 비즈니스 시너지는 충분합니다.</p>
                            </div>
                        </div>
                    </div>
                `;
                compatibilityResult.style.alignItems = 'flex-start';
                compatibilityResult.style.borderStyle = 'solid';
                compatibilityResult.style.padding = '20px';
                compatibilityResult.style.borderColor = 'rgba(255, 71, 87, 0.3)';
                compatibilityResult.style.background = 'rgba(255, 71, 87, 0.05)';

                const marriageDateInput = document.getElementById('marriage-fortune-date');
                if (marriageDateInput) {
                    marriageDateInput.onchange = (e) => {
                        const selDate = new Date(e.target.value);
                        const selYear = selDate.getFullYear();
                        const selMonth = selDate.getMonth() + 1;
                        const selDay = selDate.getDate();

                        const diffDays = Math.floor((new Date(selYear, selMonth - 1, selDay, 12, 0, 0) - new Date(2026, 3, 25, 12, 0, 0)) / (1000 * 60 * 60 * 24));
                        const tStemIdx = ((5 + diffDays) % 10 + 10) % 10;
                        const tBranchIdx = ((5 + diffDays) % 12 + 12) % 12;
                        const mStems = [
                            { h: '甲', e: 'wood' }, { h: '乙', e: 'wood' },
                            { h: '丙', e: 'fire' }, { h: '丁', e: 'fire' },
                            { h: '戊', e: 'earth' }, { h: '己', e: 'earth' },
                            { h: '庚', e: 'metal' }, { h: '辛', e: 'metal' },
                            { h: '壬', e: 'water' }, { h: '癸', e: 'water' }
                        ];
                        const mBranches = [
                            { h: '子', e: 'water' }, { h: '丑', e: 'earth' },
                            { h: '寅', e: 'wood' }, { h: '卯', e: 'wood' },
                            { h: '辰', e: 'earth' }, { h: '巳', e: 'fire' },
                            { h: '午', e: 'fire' }, { h: '未', e: 'earth' },
                            { h: '申', e: 'metal' }, { h: '酉', e: 'metal' },
                            { h: '戌', e: 'earth' }, { h: '亥', e: 'water' }
                        ];
                        const tStemObj = mStems[tStemIdx];
                        const tBranchObj = mBranches[tBranchIdx];

                        let evalText = `<strong>${selYear}년 ${selMonth}월 ${selDay}일 (${tStemObj.h}${tBranchObj.h}일)</strong>은 `;
                        if (['wood', 'fire'].includes(tStemObj.e)) {
                            evalText += `<span style="color: #ff4757; font-weight: 700;">대길 (大吉)</span>입니다. 생기와 열정의 기운이 두 분의 앞날을 축복합니다.`;
                        } else if (['earth', 'water'].includes(tStemObj.e)) {
                            evalText += `<span style="color: #2ecc71; font-weight: 700;">길 (吉)</span>합니다. 안정적이고 조화로운 흐름이 이어집니다.`;
                        } else {
                            evalText += `<span style="color: var(--text-muted); font-weight: 700;">평 (平)</span>합니다. 무난한 기운이 작용하는 날입니다.`;
                        }

                        document.getElementById('marriage-fortune-result').innerHTML = evalText;
                    };
                }
            }, 1200);
        });
    ...
    // 10. Compatibility Form Interactivity (Gender/Calendar)
...
function animateElementBars() {
    const scores = window.currentElementScores || { wood: 20, fire: 30, earth: 15, metal: 10, water: 25 };
    
    const setBar = (id, scoreId, value) => {
    ...
    setBar('bar-wood', 'score-wood', scores.wood);
    setBar('bar-fire', 'score-fire', scores.fire);
    setBar('bar-earth', 'score-earth', scores.earth);
    setBar('bar-metal', 'score-metal', scores.metal);
    setBar('bar-water', 'score-water', scores.water);
}
...
function populateDateDropdowns() {
    ...
    for (let i = currentYear; i >= 1920; i--) {
        ...
        if (i === 1990) option.selected = true; // Default to 1990
    ...
    for (let i = 1; i <= 31; i++) {
        ...
        const val = i.toString().padStart(2, '0');
        option.value = val;
        option.textContent = `${val}일`;
        if (i === 1) option.selected = true;
        daySelect.appendChild(option);
    }

    // Optional: Add logic to adjust days based on month/year selected (e.g. Feb 28/29)
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderSavedList = () => {
        ...
        if (list.length === 0) {
            savedContainer.innerHTML = `
                <div class="saju-list-empty">
                    <i class="fa-regular fa-folder-open"></i>
                    <p>저장된 사주 정보가 없습니다.</p>
                    <p style="margin-top: 6px; font-size: 0.78rem;">입력 화면에서 분석 후 저장해 주세요.</p>
                </div>
            `;
            return;
        }
        ...
        list.forEach((item, index) => {
            ...
            const badgeClass = isMale ? 'saju-list-card__badge--male' : 'saju-list-card__badge--female';
            const calText = item.calendar === 'solar' ? '양력' : item.calendar === 'lunar_leap' ? '음력(윤달)' : '음력';

            card.innerHTML = `
                <div class="saju-list-card__badge ${badgeClass}">${genderText}</div>
                <div class="saju-list-card__info" onclick="loadSajuFromList(${index})">
                    <span class="saju-list-card__name">${item.name || '익명'}</span>
                    <span class="saju-list-card__date">${item.year}년 ${item.month}월 ${item.day}일 (${calText})</span>
                </div>
